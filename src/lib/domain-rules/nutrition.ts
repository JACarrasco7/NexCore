...
export async function createNutritionPlan(input: CreateNutritionPlanInput, userId?: string): Promise<NutritionPlan> {
  try {
    ...
    await auditMutation({ entity: 'NutritionPlan', entityId: created.id, action: 'CREATE', after: created as any, userId });
...
export async function updateNutritionPlan(planId: string, payload: Partial<{ title: string; phase: string; kcalTarget: number; proteinG: number; carbsG: number; fatG: number; notes: string; isActive: boolean }>, userId?: string) {
  try {
    const before = await prisma.nutritionPlan.findUnique({ where: { id: planId } });
    if (!before) throw new BusinessError('Plan de nutrición no encontrado', ErrorCodes.NOT_FOUND, 404);

    const updated = await prisma.nutritionPlan.update({ where: { id: planId }, data: payload as any });

    await auditMutation({ entity: 'NutritionPlan', entityId: planId, action: 'UPDATE', before: before as any, after: updated as any, userId });
...
export async function activateNutritionPlan(planId: string, userId?: string) {
  try {
    ...
    await prisma.$transaction([
      prisma.nutritionPlan.updateMany({ where: { athleteId: before.athleteId }, data: { isActive: false } }),
      prisma.nutritionPlan.update({ where: { id: planId }, data: { isActive: true } }),
    ]);

    const after = await prisma.nutritionPlan.findUnique({ where: { id: planId }, include: { meals: { include: { foods: true } } } });

    await auditMutation({ entity: 'NutritionPlan', entityId: planId, action: 'UPDATE', before: before as any, after: after as any, userId });
...
export async function archiveNutritionPlan(planId: string, userId?: string) {
  try {
    ...
    const archived = await prisma.nutritionPlan.update({ where: { id: planId }, data: { deletedAt: new Date() } });

    await auditMutation({ entity: 'NutritionPlan', entityId: planId, action: 'DELETE', before: before as any, userId });
...
export async function duplicateNutritionPlan(planId: string, targetAthleteId?: string, userId?: string) {
  try {
    const original = await prisma.nutritionPlan.findUnique({ where: { id: planId }, include: { meals: { include: { foods: true } } } });
    if (!original) throw new BusinessError('Plan de nutrición no encontrado', ErrorCodes.NOT_FOUND, 404);
