...
import type { AthleteProfile, CheckInEntry, DailyLogEntry, NutritionPlan, ServicePlan, SessionLog, TrainingPlan } from "@/lib/domain";
...
export function useAthletes(coachId?: string) {
  ...
  useEffect(() => {
    ...
    apiFetch<AthleteProfile[]>(url)
      .then(setAthletes)
      .catch(console.error)
      .finally(() => setLoading(false));
  }, [coachId]);

  const addAthlete = useCallback(async (athlete: Omit<AthleteProfile, "id"> & { id?: string; coachId?: string }) => {
...
export function useTrainingPlans(athleteId?: string) {
  ...
  useEffect(() => {
    const url = athleteId ? `/api/plans?athleteId=${athleteId}` : "/api/plans";
...
export function useNutritionPlans(athleteId?: string) {
  ...
  const fetchPlans = useCallback(async () => {
    const url = athleteId
      ? `/api/nutrition-plans?athleteId=${athleteId}`
      : "/api/nutrition-plans";
...
export function useServicePlans() {
  ...
  const addPlan = useCallback(async (data: Omit<ServicePlan, "id" | "coachId" | "createdAt" | "_count">) => {
