...
export type ApiContext = {
  ...
  session: any; // Session type from @auth/core has complex requirements
  req: NextRequest;
};
  ...
  return async (
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `[API] ${req.method} ${req.nextUrl.pathname} ${durationMs}ms - 200`
        );
      }
      ...
      // Unexpected error
      console.error(
        `[API] ${req.method} ${req.nextUrl.pathname} ${durationMs}ms - 500`,
        error
      );
