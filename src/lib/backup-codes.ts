...
async function hasBackupCodesTable(): Promise<boolean> {
  try {
    const res: any = await prisma.$queryRaw`SELECT COUNT(*) as cnt FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'backup_codes'`;
...
export async function getBackupCodesForUser(userId: string): Promise<string[] | null> {
  ...
  if (table) {
    try {
      const rows: any = await prisma.$queryRaw`SELECT code_hash FROM backup_codes WHERE user_id = ${userId} ORDER BY created_at ASC`;
      if (!rows) return null;
...
export async function removeBackupCodeAtIndex(userId: string, index: number) {
  const table = await hasBackupCodesTable();
  if (table) {
    const rows: any = await prisma.$queryRaw`SELECT id FROM backup_codes WHERE user_id = ${userId} ORDER BY created_at ASC LIMIT 1 OFFSET ${index}`;
