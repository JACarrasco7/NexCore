...
export async function getCoachToday(userId: string): Promise<CoachTodayData> {
  ...
  if (!user) {
    ...
    if (session?.user?.email) {
      ...
      if (user && user.id !== userId) {
        console.warn(`[Coach] ID mismatch: session ID="${userId}" != DB ID="${user.id}". Usando DB ID.`);
      }
    }
  }
  ...
  const athleteUserIds = athletes.map((athlete) => athlete.user?.id).filter((value): value is string => Boolean(value));
  const athleteIdByUserId = new Map(
    athletes
      .filter((athlete) => athlete.user?.id)
      .map((athlete) => [athlete.user!.id, athlete.id]),
  );
  ...
  const unreadMessageRows = athleteUserIds.length > 0
  ...
  const riskAthletes = athletes
    .map((athlete) => {
      ...
      if (!lastCheckIn || lastCheckIn < tenDaysAgo) {
        const days = lastCheckIn ? Math.floor((Date.now() - lastCheckIn.getTime()) / 86_400_000) : null;
  ...
  return {
    ...
    stats: {
      ...
      avgAdherence: adherenceValues.length > 0 ? Math.round(adherenceValues.reduce((sum, value) => sum + value, 0) / adherenceValues.length) : null,
    ...
    pendingCheckIns,
    riskAthletes,
    unreadMessages: [...unreadMessagesMap.values()].sort((a, b) => (b.lastMessageAt ?? "").localeCompare(a.lastMessageAt ?? "")),
