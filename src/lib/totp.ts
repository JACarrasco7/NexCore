...
const authenticator: any = (otplib as any).authenticator || ((otplib as any).default && (otplib as any).default.authenticator) || otplib;
...
export function verifyToken(secret: string, token: string) {
  try {
    ...
    console.debug('[totp] verifyToken error', err?.message || err);
    return false;
  }
}
