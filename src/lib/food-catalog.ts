export type FoodGroup = 'carb' | 'protein' | 'fat' | 'fruit' | 'dairy' | 'legume' | 'snack'
...
export function searchFoodCatalog(query: string, limit = 8): FoodCatalogItem[] {
  ...
  return CATALOG.map((item) => ({ item, score: normalize(item.name).includes(q) ? 1 : 0 }))
    .filter((x) => x.score > 0)
    .map((x) => x.item)
    .slice(0, limit)
}
...
export function findFoodByName(name: string): FoodCatalogItem | null {
  ...
  const contains = CATALOG.find(
    (f) => normalize(f.name).includes(q) || q.includes(normalize(f.name))
  )
...
export function getEquivalences(foodName: string, quantity: number, limit = 4) {
  ...
  if (!base || quantity <= 0)
    return {
      base: null,
      items: [] as Array<{ name: string; quantity: number; unit: string; kcal: number }>,
    }

  const baseKcal = Math.max(1, Math.round(base.kcalPer100 * (quantity / 100)))
  const items = CATALOG.filter((f) => f.group === base.group && f.name !== base.name)
    .slice(0, limit)
    .map((f) => {
