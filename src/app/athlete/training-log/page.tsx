...
function ExerciseCard({
  ...
  return (
    <div className="border-line bg-surface overflow-hidden rounded-4xl border">
      {/* Header */}
      <div className="border-line flex items-start justify-between gap-4 border-b px-5 py-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {ex.videoUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="border-line text-accent hover:bg-surface-strong rounded-full border px-2 py-0.5 text-xs transition"
              >
                Demo
              </a>
            )}
            <Link
              ...
              className="border-line text-foreground/50 hover:text-foreground rounded-full border px-2 py-0.5 text-xs transition"
            >
              Ver detalle
            </Link>
          </div>
          <div className="text-foreground/55 flex flex-wrap gap-2 text-xs">
        ...
        {hasCues && (
          <button
            ...
            className="border-line text-foreground/50 hover:text-foreground shrink-0 rounded-full border px-3 py-1 text-xs transition"
          >
      ...
      {/* Coach cues panel */}
      {showCues && hasCues && (
        <div className="border-line bg-surface-strong space-y-2 border-b px-5 py-4 text-xs">
          {ex.techniqueDetail && (
            <div className="border-danger/30 bg-danger/10 text-danger rounded-xl border px-3 py-2">
              ⚡ {ex.techniqueDetail}
            </div>
          )}
          {ex.coachCue && (
            <div className="border-line bg-surface text-foreground/70 rounded-xl border px-3 py-2 italic">
              &ldquo;{ex.coachCue}&rdquo;
            </div>
          )}
          {ex.progressionNote && (
            <div className="border-success/30 bg-success/10 text-success rounded-xl border px-3 py-2">
              📈 {ex.progressionNote}
            </div>
          )}
        </div>
      ...
      <div className="px-5 py-4">
        ...
        {prevSets.length > 0 && (
          <div className="border-line bg-surface-strong mb-3 rounded-xl border px-3 py-2">
            <p className="text-foreground/40 mb-1.5 text-xs font-medium">
              Última sesión registrada
            </p>
            <div className="text-foreground/50 grid grid-cols-4 gap-2 text-xs">
              <span></span>
              <span>kg</span>
              <span>reps</span>
              <span>RIR</span>
            ...
            {prevSets.map((s, i) => (
              <div key={i} className="text-foreground/60 mt-1 grid grid-cols-4 gap-2 text-xs">
        ...
        {/* Column headers */}
        <div className="text-foreground/40 mb-2 grid grid-cols-[2rem_1fr_1fr_1fr_2.5rem] gap-2 px-1 text-xs font-medium">
          <span>#</span>
          <span>Carga (kg)</span>
          <span>Reps</span>
          <span>RIR</span>
        ...
        <div className="space-y-2">
          {entries.map((entry, setIdx) => (
            <div
              key={setIdx}
              className={`grid grid-cols-[2rem_1fr_1fr_1fr_2.5rem] gap-2 transition-opacity ${entry.done ? 'opacity-40' : ''}`}
            >
              <span
                className={`flex items-center justify-center rounded-xl border py-2 text-center text-sm font-bold ${
                  entry.done
              ...
              <input
                ...
                disabled={entry.done}
                className="border-line bg-surface-strong focus:border-accent rounded-xl border px-3 py-2 text-sm transition outline-none disabled:opacity-60"
              />
              <input
                type="number"
                value={entry.reps}
              ...
              <button
                ...
                className="border-line bg-surface-strong flex items-center justify-center rounded-xl border text-lg transition hover:border-emerald-700/60 hover:bg-emerald-900/20 hover:text-emerald-400 disabled:opacity-30"
...
function TrainingLogContent() {
  ...
  useEffect(() => {
    ...
    return () => {
      /* no limpiar aquí para que siga entre re-renders */
    }
  }, [selectedSessionId])
  ...
  const kcalEst = elapsed > 0 ? Math.round(5.5 * athleteWeightKg * (elapsed / 3600)) : 0
  ...
  function handleSessionChange(sessionId: string) {
    ...
    session.exercises.forEach((ex, i) => {
      entries[i] = buildEmptySets(ex)
    })
    setSetEntries(entries)
  }

  const updateSet = useCallback(
    (exIdx: number, setIdx: number, field: SetEntryField, value: string) => {
      setSetEntries((prev) => {
        const copy = { ...prev }
  ...
  const markSetDone = useCallback((exIdx: number, setIdx: number) => {
    setSetEntries((prev) => {
      const copy = { ...prev }
      copy[exIdx] = (copy[exIdx] ?? []).map((s, i) => (i === setIdx ? { ...s, done: true } : s))
      return copy
    })
  }, [])
  ...
  const completedSets = Object.values(setEntries)
    .flat()
    ...
    <div className="mx-auto flex w-full max-w-[1480px] gap-6 px-6 py-8 md:px-10 lg:px-12">
      <main className="flex min-w-0 flex-1 flex-col gap-8">
        ...
        <section className={`grid gap-4 ${athleteMe ? 'sm:grid-cols-2' : 'sm:grid-cols-3'}`}>
          {!athleteMe && (
            <div className="space-y-1">
              <label className="text-foreground/60 text-xs font-medium">Atleta</label>
              <select
                value={selectedAthleteId}
                onChange={(e) => handleAthleteChange(e.target.value)}
                className="border-line bg-surface focus:border-accent w-full rounded-2xl border px-3 py-2.5 text-sm transition outline-none"
              >
          ...
          <div className="space-y-1">
            <label className="text-foreground/60 text-xs font-medium">Plan</label>
            <select
              value={selectedPlanId}
              onChange={(e) => handlePlanChange(e.target.value)}
              disabled={!selectedAthleteId || loadingPlans}
              className="border-line bg-surface focus:border-accent w-full rounded-2xl border px-3 py-2.5 text-sm transition outline-none disabled:opacity-50"
            >
          ...
          <div className="space-y-1">
            <label className="text-foreground/60 text-xs font-medium">Sesión</label>
            <select
        ...
        {selectedAthleteId && !loadingPlans && plans.length === 0 && (
          <div className="border-line bg-surface flex flex-col items-center gap-4 rounded-4xl border py-16 text-center">
            ...
            <p className="text-foreground/60 text-sm">
              Importa una rutina CSV para este atleta primero.
            </p>
            <Link
        ...
        {selectedSession && !saved && (
          <div className="border-line bg-surface flex flex-wrap items-center gap-4 rounded-2xl border px-4 py-3">
            <div className="flex items-center gap-2">
              <span className="bg-success h-2 w-2 animate-pulse rounded-full" />
              <span className="font-mono text-xl font-bold tabular-nums">{fmtElapsed}</span>
              <span className="text-foreground/40 text-xs">en sesión</span>
            </div>
            <div className="bg-line hidden h-4 w-px sm:block" />
            <div className="text-foreground/50 flex items-center gap-1.5 text-xs">
              <span>~{kcalEst}</span>
              <span className="text-foreground/30">kcal estimadas</span>
              <span className="text-foreground/25 text-[10px]">(MET×peso)</span>
            </div>
            <button
              onClick={() => setShowWearable((v) => !v)}
              className="text-accent/70 hover:text-accent ml-auto text-xs transition"
            >
        ...
        {selectedSession && !saved && showWearable && (
          <div className="border-line bg-surface space-y-3 rounded-2xl border p-4">
            <div className="flex items-center gap-2">
              <span className="text-base">⌚</span>
              <p className="text-sm font-semibold">Datos del reloj / wearable</p>
              <span className="text-foreground/35 ml-auto text-[10px] tracking-widest uppercase">
                Sobrescriben la estimación
              </span>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="space-y-1">
                <label className="text-foreground/55 text-xs font-medium">kcal quemadas</label>
                <input
                  type="number"
                  min="0"
                  value={wearable.kcal}
                  ...
                  className="border-line bg-background focus:border-accent w-full rounded-xl border px-3 py-2 text-sm outline-none"
                />
              </div>
              <div className="space-y-1">
                <label className="text-foreground/55 text-xs font-medium">FC media (bpm)</label>
                <input
                  type="number"
                  min="0"
                  max="250"
              ...
              <div className="space-y-1">
                <label className="text-foreground/55 text-xs font-medium">Fuente</label>
                <select
            ...
            <p className="text-foreground/35 text-[10px]">
              Consulta el resumen de actividad en tu app del reloj y copia los valores aquí. Se
              guardan junto al registro del entrenamiento.
            </p>
          </div>
        )}
              ...
              {savedLog.kcalBurned != null && (
                <div className="rounded-xl bg-emerald-900/30 px-4 py-2 text-center">
                  ...
                  <p className="text-[11px] text-emerald-300/50">
                    {savedLog.source && savedLog.source !== 'manual'
                      ? savedLog.source
                      : 'Estimadas'}
                  </p>
                </div>
            ...
            <div className="mt-4 flex justify-center gap-3">
              <button
                onClick={() => {
                  ...
                  setSetEntries({})
                  setNotes('')
                  setWearable({ kcal: '', hr: '', source: 'manual' })
              ...
              <Link
                href={athleteMe ? `/athlete/${athleteMe.id}` : '/coach'}
          ...
          <section className="space-y-4">
            {prevSessionLog && (
              <div className="text-foreground/40 flex items-center gap-2 text-xs">
                ...
                <span>
                  Referencia:{' '}
                  {new Date(prevSessionLog.date).toLocaleDateString('es-ES', {
                    day: 'numeric',
                    month: 'short',
                  })}
                </span>
            ...
            {/* Notes + save */}
            <div className="border-line bg-surface space-y-3 rounded-4xl border p-5">
              <label className="text-foreground/70 text-sm font-medium">Notas de la sesión</label>
              <textarea
                rows={2}
                value={notes}
