...
export async function GET(
  ...
  const cadenceDays: Record<string, number> = {
    daily: 1, weekly: 7, checkin: 7, workout: 7,
  };
  const days = athlete.measurementCadence === "CUSTOM_DAYS"
  ...
  function weekStreak(dates: Date[]): number {
    ...
    const weeks = new Set(dates.map((d) => {
      ...
      const weekNo = Math.ceil(((dt.getTime() - jan4.getTime()) / 86_400_000 + jan4.getDay() + 1) / 7);
      return `${dt.getFullYear()}-${weekNo}`;
    }));
    const sorted = [...weeks].sort().reverse();
    let streak = 0;
    let prevYear = -1, prevWeek = -1;
    for (const w of sorted) {
      const [y, wk] = w.split("-").map(Number);
      if (streak === 0) {
        streak = 1;
        prevYear = y; prevWeek = wk;
        continue;
      }
  ...
  if (athlete.contextProfile) {
    const foods = Array.isArray(athlete.contextProfile.restrictedFoodsJson) ? athlete.contextProfile.restrictedFoodsJson : [];
    const exercises = Array.isArray(athlete.contextProfile.restrictedExercises) ? athlete.contextProfile.restrictedExercises : [];
    restrictionsCount = foods.length + exercises.length;
  }

  // latestWeightKg — from last check-in or last daily log
  const latestWeightKg = athlete.checkIns[0]?.weightKg
