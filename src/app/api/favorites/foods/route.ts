...
export const POST = apiHandler({
  ...
  handler: async ({ req, session }) => {
    ...
    if (!foodName?.trim() || !source?.trim())
      throw new BusinessError('foodName and source required', ErrorCodes.INVALID_INPUT, 400)

    const fav = await prisma.favoriteFood.upsert({
      where: {
        athleteId_foodName_source: {
  ...
  handler: async ({ req, session }) => {
    ...
    if (!name?.trim() || !source?.trim())
      throw new BusinessError(
        'name and source query params required',
        ErrorCodes.INVALID_INPUT,
        400
      )

    await prisma.favoriteFood.deleteMany({
      where: { athleteId: athlete.id, foodName: name, source },
    })
    return { ok: true }
  },
})
