...
async function resolveCoachPresetContext(
  athleteId: string,
  userId: string,
  ...
  if (!athlete)
    return { error: NextResponse.json({ error: 'Atleta no encontrado' }, { status: 404 }) }
...
export async function PATCH(request: Request) {
  ...
  const body = parsedBody.data as { athleteId?: string }
