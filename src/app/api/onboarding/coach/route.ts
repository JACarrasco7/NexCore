...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ id: coach.id, displayName: coach.displayName, specialty: specialty ?? null });
  } catch (error) {
    console.error("onboarding/coach POST failed", error);
    return NextResponse.json({ error: "Error interno al guardar onboarding de coach" }, { status: 500 });
  }
}
