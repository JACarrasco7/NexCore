...
export const GET = apiHandler({
  ...
  handler: async ({ req }) => {
    ...
    const takeParam =
      (req as NextRequest).nextUrl.searchParams.get('take') ??
      (req as NextRequest).nextUrl.searchParams.get('limit') ??
      undefined
    ...
    if (!paginationParams.success)
      throw new BusinessError('Invalid pagination params', ErrorCodes.VALIDATION_FAILED, 400)
...
export const POST = apiHandler({
  ...
  handler: async ({ req, session }) => {
    ...
    if (!file || !athleteId)
      throw new BusinessError('file y athleteId son requeridos', ErrorCodes.INVALID_INPUT, 400)
    ...
    const doc = await prisma.document.create({
      data: {
        ...
        category: mappedCategory as Parameters<
