...
import { resendWebhookSchema } from '@/lib/validators'
...
export async function POST(request: NextRequest) {
  ...
  const validated = resendWebhookSchema.safeParse(parsed.data)
  if (!validated.success) {
    return badRequest('Datos inválidos')
  }

  const body = validated.data as ResendWebhookPayload
  ...
  await prisma.notificationDelivery.updateMany({
    ...
    data: {
      ...
      errorMessage: status === NotificationDeliveryStatus.FAILED ? `Resend event ${event}` : null,
    },
  })
