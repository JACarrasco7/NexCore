...
export async function POST(request: NextRequest) {
  ...
  const { MessageSid: messageSid, MessageStatus: messageStatus } = parsed.data

  if (!messageSid) {
    return NextResponse.json({ error: 'MessageSid requerido' }, { status: 400 })
  }

  const status = mapTwilioStatus(messageStatus ?? null)
  ...
  await prisma.notificationDelivery.updateMany({
    ...
    data: {
      ...
      errorMessage:
        status === NotificationDeliveryStatus.FAILED ? `Twilio status ${messageStatus}` : null,
    },
  })
