...
export async function GET(request: NextRequest) {
  try {
    ...
    const { secret, otpauth } = generateSecret(String(email), "Apex Coach OS");

    // store pending secret server-side with short TTL (10 minutes)
    const expiresAt = new Date(Date.now() + 10 * 60 * 1000);
    await prisma.user.update({ where: { id: session.user.id }, data: { pendingTotpSecret: secret, pendingTotpExpiresAt: expiresAt } });
