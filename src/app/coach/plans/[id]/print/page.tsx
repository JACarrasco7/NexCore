    ...
    <>
      ...
      <div className="no-print fixed right-4 top-4 z-50 flex gap-2">
        <button
          ...
          className="rounded-xl bg-accent px-5 py-2.5 text-sm font-semibold text-white shadow-lg"
        >
        ...
        <button
          ...
          className="rounded-xl border border-line bg-white px-5 py-2.5 text-sm"
        >
          Volver
        </button>
      </div>
      ...
      <main className="mx-auto max-w-[1480px] px-6 py-12">
        ...
        <div className="mb-8 border-b pb-6">
          ...
          <div className="mt-2 flex flex-wrap gap-4 text-sm text-gray-500">
            <span>Atleta: <strong className="text-gray-800">{plan.athlete.fullName}</strong></span>
            <span>Semana: <strong className="text-gray-800">{plan.weekLabel}</strong></span>
            <span>Coach: <strong className="text-gray-800">{plan.coach.displayName}</strong></span>
          </div>
        </div>
        ...
        {plan.sessions.map((session, si) => (
          <section key={session.id} className={si > 0 ? "mt-8" : ""}>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b-2 border-gray-200 text-left text-xs uppercase tracking-wide text-gray-400">
        ...
        <p className="mt-12 text-center text-xs text-gray-300">
          Generado por ApexCoach · {new Date().toLocaleDateString("es-ES")}
        </p>
      </main>
    </>
  );
