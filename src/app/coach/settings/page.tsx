    ...
    >
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold">{goal.label}</p>
          <p className="text-foreground/50 text-xs">{goal.code}</p>
        </div>
        <div className="flex shrink-0 gap-2">
          <button
            type="button"
            onClick={() => setEditing((e) => !e)}
            className="border-line bg-surface hover:border-accent/40 rounded-xl border px-3 py-1.5 text-xs font-medium transition"
          >
            Editar
          </button>
          <button
      ...
      {editing ? (
        <div className="mt-2 space-y-2">
          <input
            ...
            className="border-line bg-surface focus:border-accent w-full rounded-2xl border px-3 py-2 text-sm outline-none"
          />
          <input
            id={`${formId}-desc`}
            value={desc}
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={save}
              className="bg-accent hover:bg-accent-strong rounded-xl px-3 py-1.5 text-xs font-semibold text-white"
            >
              Guardar
            </button>
            <button
              ...
              className="border-line hover:bg-surface-strong rounded-xl border px-3 py-1.5 text-xs font-medium"
            >
              Cancelar
            </button>
          </div>
        ...
        goal.description && <p className="text-foreground/60 text-xs">{goal.description}</p>
      )}
    </div>
  )
}
...
function PhaseRow({
  ...
  return (
    <div className="border-line bg-surface-strong flex items-center gap-3 rounded-2xl border px-4 py-3">
      {editing ? (
        <>
          <input
            id={`${formId}-label`}
            ...
            className="border-line bg-surface focus:border-accent min-w-0 flex-1 rounded-xl border px-3 py-1.5 text-sm outline-none"
            autoFocus
          />
          <button
            type="button"
        ...
        <>
          ...
          <button
            type="button"
            onClick={onDelete}
            className="text-foreground/40 hover:text-danger text-xs"
          >
            Eliminar
          </button>
        </>
...
function TeamSettingsEditor() {
  ...
  useEffect(() => {
    fetch('/api/teams/settings')
      ...
      .then(
        (
          d: {
            settings: Partial<TeamSettingsData> | null
  ...
  if (loading) {
    return <div className="border-line bg-surface h-64 animate-pulse rounded-2xl border" />
  }
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          onClick={() => void save()}
          disabled={saving}
          className="bg-accent hover:bg-accent-strong shrink-0 rounded-full px-4 py-2 text-sm font-semibold text-white transition disabled:opacity-50"
        >
      ...
      {/* Identidad */}
      <fieldset className="border-line bg-surface flex flex-col gap-3 rounded-2xl border p-4">
        <legend className="text-foreground/70 px-2 text-sm font-semibold">Identidad</legend>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <label className="flex flex-col gap-1 text-sm">
            ...
            <input
              ...
              className="border-line bg-surface-strong focus:border-accent rounded-xl border px-3 py-2 text-sm outline-none"
            />
          </label>
          <label className="flex flex-col gap-1 text-sm">
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <input
              ...
              className="border-line bg-surface-strong h-10 w-full rounded-xl border px-2"
            />
          </label>
          <label className="flex flex-col gap-1 text-sm">
      ...
      {/* Localización */}
      <fieldset className="border-line bg-surface flex flex-col gap-3 rounded-2xl border p-4">
        <legend className="text-foreground/70 px-2 text-sm font-semibold">Localización</legend>
      ...
      {/* Contacto */}
      <fieldset className="border-line bg-surface flex flex-col gap-3 rounded-2xl border p-4">
        <legend className="text-foreground/70 px-2 text-sm font-semibold">Contacto</legend>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <label className="flex flex-col gap-1 text-sm">
      ...
      {/* Defaults operativos */}
      <fieldset className="border-line bg-surface flex flex-col gap-3 rounded-2xl border p-4">
        <legend className="text-foreground/70 px-2 text-sm font-semibold">
          Defaults operativos
        </legend>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <label className="flex flex-col gap-1 text-sm">
      ...
      {/* Módulos */}
      <fieldset className="border-line bg-surface flex flex-col gap-3 rounded-2xl border p-4">
        <legend className="text-foreground/70 px-2 text-sm font-semibold">Módulos activos</legend>
        <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
          {DEFAULT_FEATURE_KEYS.map((f) => {
            ...
            return (
              <label
                key={f.key}
                className="border-line bg-surface-strong flex cursor-pointer items-center justify-between gap-2 rounded-xl border px-4 py-3 text-sm"
              >
                <span>{f.label}</span>
                <input
                  type="checkbox"
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-foreground/60 text-sm">
            Texto que el atleta debe leer y aceptar durante el onboarding. Se guarda como snapshot
            en cada firma.
          </p>
        </div>
        <button
      ...
      {loading ? (
        <div className="border-line bg-surface h-40 animate-pulse rounded-2xl border" />
      ) : (
        <textarea
          value={template}
          onChange={(e) => setTemplate(e.target.value)}
          rows={14}
          className="border-line bg-surface-strong focus:border-accent w-full resize-y rounded-2xl border px-4 py-3 font-mono text-sm transition outline-none"
...
export default function CoachSettingsPage() {
  ...
  return (
    <PageShell>
      ...
      {loading ? (
        <div className="border-line bg-surface h-40 animate-pulse rounded-3xl border" />
      ) : !teamId ? (
        <div className="border-line bg-surface rounded-3xl border border-dashed p-10 text-center">
          <p className="text-foreground/60 text-sm">
            No se encontró equipo activo. Completa el alta de un atleta primero para crear el equipo
            base.
          </p>
        </div>
      ) : (
        <div className="grid gap-8 xl:grid-cols-2">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="bg-accent hover:bg-accent-strong rounded-full px-4 py-2 text-sm font-semibold text-white transition disabled:opacity-50"
              >
              ...
              {goals.map((g) => (
                <GoalCard
                  ...
                  onEdit={(label, description) =>
                    updateGoalLocal(g.code, { label, description: description || null })
                  }
                />
              ))}
            </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <p className="text-foreground/60 text-sm">
              Crea etiquetas de fase reutilizables. En el onboarding el coach podrá elegir una en
              lugar de escribir a mano.
            </p>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="border-line bg-surface-strong focus:border-accent min-w-0 flex-1 rounded-2xl border px-4 py-3 text-sm transition outline-none"
              />
              <button
                type="button"
                onClick={() => void addPhase()}
                disabled={addingPhase || !newPhaseLabel.trim()}
                className="bg-accent hover:bg-accent-strong shrink-0 rounded-2xl px-4 py-3 text-sm font-semibold text-white transition disabled:opacity-40"
              >
                + Añadir
              </button>
            </div>
            ...
            {phases.length === 0 ? (
              <div className="border-line text-foreground/50 rounded-2xl border border-dashed p-6 text-center text-sm">
