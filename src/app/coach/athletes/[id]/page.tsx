...
type OverviewPlan = {
  id: string; title: string; weekLabel: string | null; sessionsCount: number; createdAt: string;
};
type OverviewMealFood = {
  id: string; food: string; quantity: number; unit: string;
  kcal: number | null; proteinG: number | null; carbsG: number | null; fatG: number | null;
};
type OverviewMeal = {
  id: string; name: string; time: string; order: number;
  foods: OverviewMealFood[];
};
type OverviewNutritionPlan = {
  id: string; title: string; isActive: boolean;
  kcalTarget: number | null; proteinG: number | null; carbsG: number | null; fatG: number | null;
  meals: OverviewMeal[];
};
type OverviewCheckIn = {
  id: string; date: string; weekLabel: string | null;
  weightKg: number; adherencePct: number; sleepHours: number; stepsAvg: number;
  sensations?: string | null; coachNote?: string | null;
};
type OverviewSession = {
  id: string; sessionName: string; date: string;
  durationMin: number | null; kcalBurned: number | null; heartRateAvg?: number | null; source?: string;
};
type OverviewDoc = {
  id: string; title: string; category: string; fileName: string; createdAt: string;
};
...
type BodyMeasurement = {
  ...
  armCm: number | null; quadCm: number | null; calfCm: number | null;
  glutesCm: number | null; neckCm: number | null; notes: string | null;
};
type Overview = {
  id: string;
  fullName: string;
...
function AdherenceBar({ pct }: { pct: number }) {
  ...
  return (
    <div className="h-1.5 w-full rounded-full bg-line/30">
...
function KpiChip({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-surface-strong px-2.5 py-2 text-center">
      <p className="text-[10px] uppercase tracking-widest text-foreground/35">{label}</p>
...
function ActionLink({ href, icon, label }: { href: string; icon: string; label: string }) {
  return (
    <Link href={href}
      className="flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-foreground/60 transition hover:bg-surface-strong hover:text-foreground">
...
function NutriBadge({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded bg-surface-strong px-2 py-1">
...
function StatBig({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-2xl border border-line bg-surface p-4 text-center">
      ...
      {sub && <p className="mt-0.5 text-[10px] text-foreground/30">{sub}</p>}
    </div>
  );
}
...
function EmptyCard({ text, tall }: { text: string; tall?: boolean }) {
  return (
    <div className={`flex items-center justify-center rounded-2xl border border-dashed border-line bg-surface text-sm text-foreground/40 ${tall ? "py-16" : "p-8"}`}>
      {text}
    </div>
  );
}
...
function AsideContent({
    ...
    <div className="rounded-2xl border border-line bg-surface p-4 shadow-sm">
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0">
          <p className="truncate font-semibold leading-tight">{overview.fullName}</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        ...
        <KpiChip label="Peso" value={stats.latestWeightKg != null ? `${stats.latestWeightKg} kg` : "—"} />
  ...
  const quickActions = (
    <div className="rounded-2xl border border-line bg-surface p-3 shadow-sm space-y-1">
      <p className="px-1 pb-1 text-[10px] uppercase tracking-widest text-foreground/35">Acciones</p>
      <ActionLink href={`/coach/messages?athlete=${athleteId}`} icon="💬" label="Enviar mensaje" />
      <ActionLink href={`/coach/athletes/${athleteId}/context/print`} icon="🖨️" label="Imprimir contexto" />
      <ActionLink href={`/coach/nutrition?athlete=${athleteId}`} icon="🥗" label="Plan nutricional" />
    </div>
  );
    ...
    return (
      <>
        ...
        {daysNoCI != null && daysNoCI > 7 && (
          <div className="rounded-2xl border border-warning/30 bg-warning/8 p-3 text-xs text-warning">
  ...
  if (tab === "estadisticas") {
    return (
      <>
        {identityCard}
        <div className="rounded-2xl border border-line bg-surface p-4 shadow-sm">
          <p className="mb-2 text-xs font-semibold text-foreground/50">Tendencia peso</p>
  ...
  if (tab === "checkins") {
    return (
      <>
        {identityCard}
        {lastCI && (
          <div className="rounded-2xl border border-line bg-surface p-4 shadow-sm space-y-2">
            <p className="text-xs font-semibold text-foreground/45">Último check-in</p>
            <p className="font-semibold">{lastCI.weekLabel ?? lastCI.date}</p>
            <AdherenceBar pct={lastCI.adherencePct} />
            <p className="text-xs text-foreground/50">{lastCI.adherencePct}% · {lastCI.weightKg} kg</p>
          </div>
        )}
        {stats.nextCheckInDueAt && (
          <div className="rounded-2xl border border-line bg-surface px-4 py-3 text-xs shadow-sm">
  ...
  if (tab === "entrenamiento") {
    return (
      <>
        ...
        {activePlan ? (
          <div className="rounded-2xl border border-line bg-surface p-4 shadow-sm space-y-2">
            <p className="text-[10px] uppercase tracking-widest text-foreground/35">Plan activo</p>
            <p className="font-semibold">{activePlan.title}</p>
            {activePlan.weekLabel && <p className="text-xs text-foreground/50">{activePlan.weekLabel}</p>}
            <p className="text-xs text-foreground/50">{activePlan.sessionsCount} sesiones</p>
            <Link href={`/coach/plans/${activePlan.id}/print`}
              className="mt-1 block rounded-xl border border-line py-1.5 text-center text-xs text-foreground/60 transition hover:text-foreground">
              Imprimir plan
            </Link>
          </div>
        ) : (
          <p className="rounded-2xl border border-dashed border-line p-4 text-center text-xs text-foreground/40">
            Sin plan activo
          </p>
        )}
      </>
  ...
  if (tab === "contexto") {
    return (
      <>
        {identityCard}
        <div className="rounded-2xl border border-line bg-surface p-4 shadow-sm">
          <p className="text-[10px] uppercase tracking-widest text-foreground/35">Restricciones</p>
          <p className="mt-1 text-2xl font-bold">{stats.restrictionsCount}</p>
          <p className="text-xs text-foreground/45">alimentos + ejercicios</p>
        </div>
      </>
    );
  }
  ...
  if (tab === "nutricion") {
    return (
      <>
        ...
        <Link href={`/coach/nutrition?athlete=${athleteId}`}
          className="block rounded-xl bg-accent px-4 py-2.5 text-center text-sm font-semibold text-white transition hover:bg-accent-strong">
          Editar plan
        </Link>
      </>
    );
  ...
  if (tab === "documentos") {
    ...
    return (
      <>
        {identityCard}
        <div className="rounded-2xl border border-line bg-surface p-4 shadow-sm space-y-2">
          <p className="text-[10px] uppercase tracking-widest text-foreground/35">Documentos</p>
          <p className="mt-1 text-2xl font-bold">{overview.documents.length}</p>
          {cats.map((c) => (
            <div key={c} className="flex items-center justify-between text-xs">
              <span className="capitalize text-foreground/60">{c}</span>
              <span className="text-foreground/40">{overview.documents.filter((d) => d.category === c).length}</span>
            </div>
          ))}
        </div>
      </>
  ...
  return <>{identityCard}{quickActions}</>;
}
...
function ChartCard({ title, children, footnote }: { title: string; children: React.ReactNode; footnote?: string }) {
  return (
    <div className="rounded-2xl border border-line bg-surface p-5">
      <h3 className="mb-4 font-semibold text-foreground/70">{title}</h3>
      {children}
      {footnote && <p className="mt-2 text-xs text-foreground/40">{footnote}</p>}
    </div>
  );
}
...
function BodyCompositionTrendChart({ measurements }: { measurements: BodyMeasurement[] }) {
  const data = measurements
    ...
    .map((m) => {
      ...
      return { label: new Date(m.date).toLocaleDateString("es-ES", { day: "numeric", month: "short" }), weight, fatMass, leanMass };
    });
    ...
    <ChartCard title="Composición corporal (peso, masa magra y grasa)">
      <ResponsiveContainer width="100%" height={250}>
        <LineChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
          ...
          <Line type="monotone" dataKey="weight" name="Peso" stroke="var(--color-accent)" strokeWidth={2.5} dot={{ r: 3 }} />
          <Line type="monotone" dataKey="leanMass" name="Masa magra" stroke="#34d399" strokeWidth={2} dot={{ r: 2 }} connectNulls />
          <Line type="monotone" dataKey="fatMass" name="Masa grasa" stroke="#fb923c" strokeWidth={1.5} strokeDasharray="4 4" dot={{ r: 2 }} connectNulls />
        </LineChart>
      </ResponsiveContainer>
    </ChartCard>
  );
...
function AdvancedMetricsPanel({ overview }: { overview: Overview }) {
  ...
  const lastWeight = overview.weightTrend.slice().reverse().find((w) => w.weightKg != null)?.weightKg ?? null;
  const avgSteps7 = overview.stepsTrend.length >= 7
    ? Math.round(overview.stepsTrend.slice(-7).reduce((s, d) => s + d.value, 0) / 7)
    : (overview.stepsTrend.length > 0
      ? Math.round(overview.stepsTrend.reduce((s, d) => s + d.value, 0) / overview.stepsTrend.length)
      : null);

  const neat = avgSteps7 != null && lastWeight != null ? estimateNeatFromSteps(avgSteps7, lastWeight) : null;
  const bf = lastMeasure?.bodyFatPct ?? null;
  const heightM = 1.74;
  const ffmi = lastWeight != null && bf != null
  ...
  const waistTrend = overview.bodyMeasurements
    ...
    .slice(-8)
    .map((m) => ({ label: new Date(m.date).toLocaleDateString("es-ES", { day: "numeric", month: "short" }), value: m.waistCm as number }));
    ...
    <ChartCard title="Métricas avanzadas" footnote="NEAT, FFMI y control de composición">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-xl border border-line bg-surface-strong p-3">
          <p className="text-[10px] uppercase tracking-widest text-foreground/40">NEAT estimado</p>
          <p className="mt-1 text-xl font-bold text-foreground">{neat != null ? `${neat.toLocaleString("es-ES")} kcal` : "—"}</p>
          <p className="text-xs text-foreground/40">{avgSteps7 != null ? `${avgSteps7.toLocaleString("es-ES")} pasos/día (7d)` : "Sin pasos"}</p>
        </div>
        <div className="rounded-xl border border-line bg-surface-strong p-3">
          <p className="text-[10px] uppercase tracking-widest text-foreground/40">FFMI (est.)</p>
          <p className="mt-1 text-xl font-bold text-foreground">{ffmi ?? "—"}</p>
          <p className="text-xs text-foreground/40">Altura ref 1.74m</p>
        </div>
        <div className="rounded-xl border border-line bg-surface-strong p-3">
          <p className="text-[10px] uppercase tracking-widest text-foreground/40">% grasa</p>
          <p className="mt-1 text-xl font-bold text-foreground">{bf != null ? `${bf}%` : "—"}</p>
          <p className="text-xs text-foreground/40">Última medición</p>
        </div>
        <div className="rounded-xl border border-line bg-surface-strong p-3">
          <p className="text-[10px] uppercase tracking-widest text-foreground/40">Cintura</p>
          <p className="mt-1 text-xl font-bold text-foreground">{lastMeasure?.waistCm != null ? `${lastMeasure.waistCm} cm` : "—"}</p>
          <p className="text-xs text-foreground/40">Proxy grasa visceral</p>
        </div>
      </div>
        ...
        <div className="mt-4">
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={waistTrend} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="value" stroke="#fb7185" strokeWidth={2} fill="url(#waistGradAdv)" dot={{ r: 2 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
...
function CoachSignalsPanel({ overview }: { overview: Overview }) {
  ...
  if (lastCheckIn) {
    if (lastCheckIn.adherencePct < 60) signals.push({ level: "alert", msg: `Adherencia crítica (${lastCheckIn.adherencePct}%)` });
    else if (lastCheckIn.adherencePct < 85) signals.push({ level: "warn", msg: `Adherencia mejorable (${lastCheckIn.adherencePct}%)` });
  ...
  if (steps7.length >= 3) {
    ...
    if (avgSteps < 7000) signals.push({ level: "alert", msg: `Actividad baja (${avgSteps.toLocaleString("es-ES")} pasos)` });
    else if (avgSteps < 10000) signals.push({ level: "warn", msg: `Actividad media (${avgSteps.toLocaleString("es-ES")} pasos)` });
    else signals.push({ level: "ok", msg: `Actividad buena (${avgSteps.toLocaleString("es-ES")} pasos)` });
  }
  ...
  return (
    <ChartCard title="Señales del coach" footnote="Alertas automáticas por adherencia, sueño y pasos">
      <div className="space-y-2">
        {signals.map((s, i) => (
          <div key={`${s.level}-${i}`} className={`rounded-xl border px-3 py-2 text-sm ${cls(s.level)}`}>
            {s.msg}
          </div>
        ))}
      </div>
    ...
    <ChartCard title="Tendencia de check-ins" footnote="Adherencia, sueño y peso por semana">
      <ResponsiveContainer width="100%" height={250}>
        <ComposedChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
          <CartesianGrid {...CHART_GRID} />
          <XAxis dataKey="label" tick={{ fontSize: 10, fill: "rgba(255,255,255,0.4)" }} axisLine={false} tickLine={false} />
          <YAxis yAxisId="pct" domain={[0, 100]} tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="%" />
          <YAxis yAxisId="kg" orientation="right" tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="kg" />
          <Tooltip {...TOOLTIP_STYLE} />
          <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }} />
          <Line yAxisId="pct" type="monotone" dataKey="adherencia" name="Adherencia %" stroke="#34d399" strokeWidth={2} dot={{ r: 3 }} />
          <Line yAxisId="pct" type="monotone" dataKey="sueno" name="Sueño (h)" stroke="#f59e0b" strokeWidth={1.5} strokeDasharray="4 3" dot={{ r: 2 }} />
          <Line yAxisId="kg" type="monotone" dataKey="peso" name="Peso (kg)" stroke="#a78bfa" strokeWidth={2.5} dot={{ r: 3 }} />
        </ComposedChart>
      </ResponsiveContainer>
    </ChartCard>
  );
    ...
    <ChartCard title="Duración y kcal por sesión" footnote="Carga interna por sesión registrada">
      <ResponsiveContainer width="100%" height={250}>
        <ComposedChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
          ...
          <YAxis yAxisId="kcal" orientation="right" tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="k" />
          <Tooltip {...TOOLTIP_STYLE} />
          <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }} />
          <Bar yAxisId="dur" dataKey="duration" name="Duración (min)" fill="rgba(56,189,248,0.45)" radius={[4, 4, 0, 0]} />
          <Line yAxisId="kcal" type="monotone" dataKey="kcal" name="Kcal" stroke="#fb923c" strokeWidth={2} dot={{ r: 2 }} connectNulls />
        </ComposedChart>
      </ResponsiveContainer>
    </ChartCard>
  );
      ...
      <ResponsiveContainer width="100%" height={240}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
          ...
          <Bar dataKey="hr" radius={[4, 4, 0, 0]}>
            {data.map((entry, i) => <Cell key={i} fill={hrColor(entry.hr)} />)}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </ChartCard>
...
function RIRComplianceChart({ trainingSessions }: { trainingSessions: TrainingStatsSession[] }) {
  const data = trainingSessions
    ...
    .map((s) => {
      ...
      const avgRir = rirs.length ? +(rirs.reduce((a, b) => a + b, 0) / rirs.length).toFixed(1) : null;
      return { label: new Date(s.date).toLocaleDateString("es-ES", { day: "numeric", month: "short" }), avgRir, session: s.sessionName, totalSets: s.sets.length };
    })
    .filter((d): d is { label: string; avgRir: number; session: string; totalSets: number } => d.avgRir != null);

  if (data.length === 0) return null;

  const overallAvg = +(data.reduce((s, d) => s + d.avgRir, 0) / data.length).toFixed(1);
  const rirColor = (rir: number) => rir <= 1 ? "#22c55e" : rir <= 2 ? "#14b8a6" : rir <= 3 ? "#f59e0b" : "#ef4444";
      ...
      <ResponsiveContainer width="100%" height={230}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
          ...
          <Bar dataKey="avgRir" radius={[4, 4, 0, 0]}>
            {data.map((entry, i) => <Cell key={i} fill={rirColor(entry.avgRir)} />)}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </ChartCard>
    ...
    <ChartCard title="Volumen acumulado por semana" footnote={`Media ${avgSets} series/sem`}>
      <ResponsiveContainer width="100%" height={230}>
        <ComposedChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
          ...
          <Area type="monotone" dataKey="sets" stroke="var(--color-accent)" fill="url(#grad-meso)" strokeWidth={2.2} />
        </ComposedChart>
      </ResponsiveContainer>
    </ChartCard>
  );
...
function TonnageChart({ trainingSessions }: { trainingSessions: TrainingStatsSession[] }) {
  ...
  const data = Array.from(weekMap.entries())
    .sort((a, b) => a[0].localeCompare(b[0]))
    .map(([date, e]) => ({ label: date.slice(5), tonnage: Math.round(e.tonnage), sessions: e.sessions }));
    ...
    <ChartCard title="Tonelaje semanal" footnote="kg × reps acumulado por semana">
      <ResponsiveContainer width="100%" height={230}>
        <ComposedChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
          ...
          <CartesianGrid {...CHART_GRID} vertical={false} />
          <XAxis dataKey="label" tick={{ fontSize: 10, fill: "rgba(255,255,255,0.4)" }} axisLine={false} tickLine={false} />
          <YAxis tick={AXIS_STYLE} axisLine={false} tickLine={false} tickFormatter={(v) => `${Math.round(Number(v) / 1000)}t`} />
          <Tooltip {...TOOLTIP_STYLE} formatter={(v) => [`${Number(v).toLocaleString("es-ES")} kg`, "Tonelaje"]} />
          <Area type="monotone" dataKey="tonnage" stroke="#fbbf24" fill="url(#grad-ton)" strokeWidth={2.2} />
        </ComposedChart>
      </ResponsiveContainer>
    </ChartCard>
  );
...
function LoadProgressionChart({ trainingSessions }: { trainingSessions: TrainingStatsSession[] }) {
  const exerciseNames = Array.from(new Set(trainingSessions.flatMap((s) => s.sets.map((st) => st.exercise)))).sort();
  ...
  const data = trainingSessions
    ...
    .map((s) => {
      ...
      const avgLoad = +(relevant.reduce((acc, st) => acc + st.loadKg, 0) / relevant.length).toFixed(1);
      const maxLoad = Math.max(...relevant.map((st) => st.loadKg));
      return { label: new Date(s.date).toLocaleDateString("es-ES", { day: "numeric", month: "short" }), avgLoad, maxLoad, sets: relevant.length };
    })
    .filter((d): d is { label: string; avgLoad: number; maxLoad: number; sets: number } => d !== null);
    ...
    <ChartCard title="Progresión de carga" footnote="Carga media y máxima por ejercicio">
      <div className="mb-3">
        <select
          ...
          className="rounded-xl border border-line bg-surface px-3 py-1.5 text-xs text-foreground outline-none focus:border-accent"
        >
          {exerciseNames.map((n) => <option key={n} value={n}>{n}</option>)}
        </select>
      </div>
      {data.length < 2 ? (
        <p className="text-xs text-foreground/35">Se necesitan al menos 2 sesiones con este ejercicio.</p>
      ) : (
        <ResponsiveContainer width="100%" height={230}>
          <LineChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
            <CartesianGrid {...CHART_GRID} vertical={false} />
            ...
            <Tooltip {...TOOLTIP_STYLE} />
            <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }} />
            <Line type="monotone" dataKey="avgLoad" name="Media kg" stroke="#a78bfa" strokeWidth={2.5} dot={{ r: 3 }} />
            <Line type="monotone" dataKey="maxLoad" name="Máx kg" stroke="#38bdf8" strokeWidth={1.5} strokeDasharray="4 4" dot={{ r: 2 }} />
          </LineChart>
        </ResponsiveContainer>
      )}
    </ChartCard>
...
function E1RMChart({ trainingSessions }: { trainingSessions: TrainingStatsSession[] }) {
  ...
  const data = trainingSessions
    .filter((s) => s.sets.some((st) => st.exercise === selected))
    .map((s) => {
      const relevant = s.sets.filter((st) => st.exercise === selected && st.loadKg > 0 && st.reps > 0);
      if (!relevant.length) return null;
      ...
      return { label: new Date(s.date).toLocaleDateString("es-ES", { day: "numeric", month: "short" }), best, avg: avgE };
    })
      ...
      {data.length < 2 ? (
        ...
        <ResponsiveContainer width="100%" height={230}>
          <LineChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
            ...
            <Tooltip {...TOOLTIP_STYLE} />
            <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }} />
            <Line type="monotone" dataKey="best" name="e1RM máx" stroke="#fbbf24" strokeWidth={2.5} dot={{ r: 3 }} />
            <Line type="monotone" dataKey="avg" name="e1RM med" stroke="#a78bfa" strokeWidth={1.5} strokeDasharray="4 4" dot={{ r: 2 }} />
          </LineChart>
        </ResponsiveContainer>
      )}
    </ChartCard>
...
function MuscleVolumeProgressionChart({ trainingSessions }: { trainingSessions: TrainingStatsSession[] }) {
  ...
  const allMuscles = Array.from(new Set(Array.from(weekMuscleMap.values()).flatMap((m) => Array.from(m.keys()))));
  const muscleTotals = allMuscles
    .map((m) => ({ muscle: m, total: weeks.reduce((sum, wk) => sum + ((weekMuscleMap.get(wk) as Map<string, number>)?.get(m) ?? 0), 0) }))
  ...
  return (
    <ChartCard title="Progresión de volumen por músculo" footnote="Series por semana en los grupos más trabajados">
      <ResponsiveContainer width="100%" height={240}>
...
function MuscleRadarCoverage({ seriesByMuscle }: { seriesByMuscle: Array<{ name: string; series: number }> }) {
  if (seriesByMuscle.length < 3) return null;
  const max = Math.max(...seriesByMuscle.map((m) => m.series), 1);
  const data = seriesByMuscle.map((m) => ({ muscle: m.name, coverage: Math.round((m.series / max) * 100), target: 70 }));

  return (
    <ChartCard title="Radar de cobertura muscular" footnote="% relativo de series por grupo en el rango seleccionado">
      <ResponsiveContainer width="100%" height={300}>
        <RadarChart data={data} margin={{ top: 16, right: 20, bottom: 16, left: 20 }}>
          <PolarGrid stroke="rgba(148,163,184,0.12)" />
          <PolarAngleAxis dataKey="muscle" tick={{ fill: "rgba(255,255,255,0.65)", fontSize: 10 }} />
          <PolarRadiusAxis angle={90} domain={[0, 100]} tick={{ fill: "rgba(255,255,255,0.35)", fontSize: 9 }} />
          <Radar name="Objetivo base" dataKey="target" stroke="rgba(148,163,184,0.4)" fill="rgba(148,163,184,0.08)" strokeDasharray="4 3" />
          <Radar name="Actual" dataKey="coverage" stroke="var(--color-accent)" fill="rgba(99,102,241,0.2)" strokeWidth={2.5} />
          <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }} />
...
function MiniBar({ value, max, color, label, sublabel }: {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className="flex h-24 w-full items-end rounded-xl bg-background/70 px-1 py-1">
        <div
      ...
      <p className="text-[10px] font-semibold text-foreground/70 text-center">{sublabel ?? value}</p>
      <p className="text-[9px] uppercase tracking-widest text-foreground/35 text-center truncate w-full">{label}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatBig label="Peso actual" value={stats.latestWeightKg != null ? `${stats.latestWeightKg} kg` : "—"} />
        <StatBig label="% Grasa" value={latestM?.bodyFatPct != null ? `${latestM.bodyFatPct}%` : "—"} />
        ...
        <ChartCard title="Evolución de peso" footnote={`${weightSeries.length} registros`}>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={weightSeries} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <CartesianGrid {...CHART_GRID} />
              <XAxis dataKey="label" tick={AXIS_STYLE} axisLine={false} tickLine={false} />
              <YAxis domain={["auto", "auto"]} tick={AXIS_STYLE} axisLine={false} tickLine={false} unit=" kg" />
              <Tooltip {...TOOLTIP_STYLE} formatter={(v) => [`${v} kg`, "Peso"]} />
              <Area type="monotone" dataKey="value" name="Peso" stroke="var(--color-accent)" strokeWidth={2} fill="url(#wGrad)" dot={{ r: 3, fill: "var(--color-accent)" }} />
            </AreaChart>
          </ResponsiveContainer>
        </ChartCard>
      )}
        ...
        <ChartCard title="% Grasa corporal" footnote={`${fatSeries.length} mediciones`}>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={fatSeries} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <CartesianGrid {...CHART_GRID} />
              <XAxis dataKey="label" tick={AXIS_STYLE} axisLine={false} tickLine={false} />
              <YAxis domain={[0, "auto"]} tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="%" />
              <Tooltip {...TOOLTIP_STYLE} formatter={(v) => [`${v}%`, "% grasa"]} />
              <Area type="monotone" dataKey="value" name="% Grasa" stroke="#f59e0b" strokeWidth={2} fill="url(#bfGrad)" dot={{ r: 3, fill: "#f59e0b" }} />
            </AreaChart>
          </ResponsiveContainer>
        </ChartCard>
      )}
      ...
      {latestM && (
        <ChartCard title="Medidas actuales">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {measures.filter((m) => m.value != null).map((m) => (
              <div key={m.label} className="rounded-xl bg-surface-strong p-3 text-center">
                <p className="text-xl font-bold">{m.value}{m.suffix}</p>
                <p className="mt-0.5 text-xs text-foreground/45">{m.label}</p>
              </div>
            ))}
          </div>
        </ChartCard>
        ...
        <ChartCard title="Evolución de cintura" footnote="Tendencia de medida de cintura (cm)">
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={waistSeries} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              <CartesianGrid {...CHART_GRID} />
              <XAxis dataKey="label" tick={AXIS_STYLE} axisLine={false} tickLine={false} />
              <YAxis domain={["auto", "auto"]} tick={AXIS_STYLE} axisLine={false} tickLine={false} unit=" cm" />
              <Tooltip {...TOOLTIP_STYLE} formatter={(v) => [`${v} cm`, "Cintura"]} />
              <Line type="monotone" dataKey="value" name="Cintura" stroke="#fb7185" strokeWidth={2} dot={{ r: 3, fill: "#fb7185" }} />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
      )}
      ...
      {weightSeries.length === 0 && fatSeries.length === 0 && !latestM && (
        <div className="rounded-2xl border border-line bg-surface py-12 text-center text-sm text-foreground/40">
...
function SubtabAdherencia({ overview }: { overview: Overview }) {
  ...
  const adherenciaSeries = [...checkIns].reverse().slice(-20).map((ci) => ({
  ...
  return (
    <div className="space-y-4">
      ...
      {adherenciaSeries.length > 1 && (
        <ChartCard title="Adherencia y sueño por check-in" footnote={`Últimos ${adherenciaSeries.length} check-ins`}>
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={adherenciaSeries} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
              <CartesianGrid {...CHART_GRID} />
              <XAxis dataKey="label" tick={{ fontSize: 10, fill: "rgba(255,255,255,0.4)" }} axisLine={false} tickLine={false} />
              <YAxis yAxisId="left" domain={[0, 100]} tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="%" />
              <YAxis yAxisId="right" orientation="right" domain={[0, 12]} tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="h" />
              <Tooltip {...TOOLTIP_STYLE} />
              <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }} />
              <Line yAxisId="left" type="monotone" dataKey="adherencia" name="Adherencia %" stroke="var(--color-accent)" strokeWidth={2} dot={{ r: 3 }} />
              <Line yAxisId="right" type="monotone" dataKey="sueno" name="Sueño (h)" stroke="#34d399" strokeWidth={2} dot={{ r: 3 }} strokeDasharray="5 3" />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
      )}
      ...
      {sleepSeries.length > 1 && (
        <ChartCard title="Sueño diario (horas)" footnote={`${sleepSeries.length} registros · media ${avgSleep} h`}>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={sleepSeries} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <CartesianGrid {...CHART_GRID} />
              <XAxis dataKey="label" tick={AXIS_STYLE} axisLine={false} tickLine={false} />
              <YAxis domain={[0, 12]} tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="h" />
              <Tooltip {...TOOLTIP_STYLE} formatter={(v) => [`${v} h`, "Sueño"]} />
              <Area type="monotone" dataKey="value" name="Sueño" stroke="#34d399" strokeWidth={2} fill="url(#sleepGrad)" dot={{ r: 3, fill: "#34d399" }} />
            </AreaChart>
          </ResponsiveContainer>
        </ChartCard>
      )}
      ...
      {stepsSeries.length > 1 && (
        <ChartCard title="Pasos diarios" footnote={`${stepsSeries.length} registros · media ${avgSteps?.toLocaleString("es-ES")}`}>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={stepsSeries} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              <CartesianGrid {...CHART_GRID} />
              <XAxis dataKey="label" tick={AXIS_STYLE} axisLine={false} tickLine={false} />
              <YAxis tick={AXIS_STYLE} axisLine={false} tickLine={false} />
              <Tooltip {...TOOLTIP_STYLE} formatter={(v) => [Number(v).toLocaleString("es-ES"), "Pasos"]} />
      ...
      {correlationSeries.filter((p) => p.sueno > 0 || p.entrenamiento > 0).length > 2 && (
        <ChartCard title="Sueño vs entrenamiento" footnote="Correlación horas de descanso con minutos de entreno">
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={correlationSeries} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
              <CartesianGrid {...CHART_GRID} />
              <XAxis dataKey="label" tick={{ fontSize: 10, fill: "rgba(255,255,255,0.4)" }} axisLine={false} tickLine={false} />
              <YAxis yAxisId="sleep" domain={[0, 12]} tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="h" />
              <YAxis yAxisId="training" orientation="right" tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="min" />
              <Tooltip {...TOOLTIP_STYLE} />
              <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }} />
              <Line yAxisId="sleep" type="monotone" dataKey="sueno" name="Sueño (h)" stroke="#34d399" strokeWidth={2} dot={{ r: 3 }} />
...
function SubtabEntrenamiento({ overview }: { overview: Overview }) {
  ...
  const weekSeries = Object.entries(byWeek)
    ...
    .slice(-12)
    .map(([k, v]) => ({ label: k.split("-")[1], sesiones: v.count, minutos: v.totalMin, kcal: v.totalKcal }));
  ...
  const avgDuration = sessions.filter((s) => s.durationMin).length
    ? Math.round(sessions.filter((s) => s.durationMin).reduce((a, s) => a + (s.durationMin ?? 0), 0) / sessions.filter((s) => s.durationMin).length)
    : null;
  const totalKcal = sessions.reduce((a, s) => a + (s.kcalBurned ?? 0), 0);
  const avgHR = sessions.filter((s) => s.heartRateAvg).length
    ? Math.round(sessions.filter((s) => s.heartRateAvg).reduce((a, s) => a + (s.heartRateAvg ?? 0), 0) / sessions.filter((s) => s.heartRateAvg).length)
    : null;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatBig label="Kcal quemadas" value={totalKcal > 0 ? totalKcal.toLocaleString("es-ES") : "—"} />
      ...
      {trainingStatsLoading && (
        <div className="rounded-2xl border border-line bg-surface py-6 text-center text-xs text-foreground/40">
        ...
        <ChartCard title="Volumen semanal" footnote={`Últimas ${weekSeries.length} semanas`}>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={weekSeries} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <YAxis yAxisId="right" orientation="right" tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="min" />
              <Tooltip {...TOOLTIP_STYLE} />
              <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }} />
              <Bar yAxisId="left" dataKey="sesiones" name="Sesiones" fill="var(--color-accent)" radius={[4, 4, 0, 0]} opacity={0.85} />
              <Bar yAxisId="right" dataKey="minutos" name="Minutos" fill="#34d399" radius={[4, 4, 0, 0]} opacity={0.6} />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
      )}
        ...
        <ChartCard title="Sueño vs minutos de entreno" footnote="Por día con sesión registrada">
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={correlSeries} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
              ...
              <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }} />
              <Line yAxisId="sleep" type="monotone" dataKey="sueno" name="Sueño (h)" stroke="#34d399" strokeWidth={2} dot={{ r: 3 }} />
              <Line yAxisId="training" type="monotone" dataKey="minutos" name="Entreno (min)" stroke="var(--color-accent)" strokeWidth={2} dot={{ r: 3 }} strokeDasharray="5 3" />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
      )}
        ...
        <ChartCard title="Últimas sesiones">
          <div className="space-y-1.5">
            {sessions.slice(-12).reverse().map((s) => (
              <div key={s.id} className="flex items-center justify-between rounded-xl bg-surface-strong px-3 py-2 text-sm">
                <div>
                  <p className="font-medium">{s.sessionName || "Sesión"}</p>
                  <p className="text-xs text-foreground/40">{s.date}</p>
                </div>
...
const MUSCLE_MAP: Record<string, string> = {
  // ─ Pectoral
  "press banca": "Pectoral", "press inclinado": "Pectoral", "press plano": "Pectoral",
  "press declinado": "Pectoral", "cable crossover": "Pectoral", "pec deck": "Pectoral",
  "aperturas": "Pectoral", "fondos": "Pectoral", "bench": "Pectoral", "pec": "Pectoral",
  // ─ Deltoides (antes de "press" genérico)
  "press militar": "Deltoides", "press arnold": "Deltoides",
  "elevaciones laterales": "Deltoides", "elevaciones frontales": "Deltoides",
  "hombro": "Deltoides", "deltoides": "Deltoides",
  "elevaciones": "Deltoides", "lateral": "Deltoides", "posterior": "Deltoides", "face pull": "Deltoides",
  // ─ Pectoral generic fallback
  "press": "Pectoral",
  // ─ Espalda
  "remo": "Espalda", "dominadas": "Espalda", "jalón": "Espalda", "jalon": "Espalda",
  "pulldown": "Espalda", "pullover": "Espalda", "renegade": "Espalda", "hiperextension": "Espalda",
  "peso muerto": "Espalda", "deadlift": "Espalda",
  // ─ Trapecio
  "trapecio": "Trapecio", "encogimiento": "Trapecio", "shrug": "Trapecio",
  // ─ Bíceps (curl martillo antes de curl genérico)
  "curl martillo": "Bíceps", "curl": "Bíceps", "bicep": "Bíceps", "bíceps": "Bíceps", "martillo": "Bíceps",
  // ─ Tríceps (press francés antes de extensión genérica)
  "press francés": "Tríceps", "pushdown": "Tríceps", "tríceps": "Tríceps", "tricep": "Tríceps", "dips": "Tríceps",
  // ─ Cuádriceps (específicos antes de genéricos)
  "leg press": "Cuádriceps", "extensión de cuádr": "Cuádriceps", "extensión cuádr": "Cuádriceps",
  "sentadilla": "Cuádriceps", "squat": "Cuádriceps", "prensa": "Cuádriceps",
  "cuádricep": "Cuádriceps", "abductores": "Cuádriceps", "leg": "Cuádriceps",
  // ─ Isquiotibiales
  "curl femoral": "Isquiotibiales", "femoral": "Isquiotibiales",
  "peso rumano": "Isquiotibiales", "rdl": "Isquiotibiales", "isquio": "Isquiotibiales",
  // ─ Glúteos
  "hip thrust": "Glúteos", "hip": "Glúteos",
  "glúteo": "Glúteos", "glutes": "Glúteos", "lunges": "Glúteos", "zancada": "Glúteos",
  // ─ Gemelos
  "pantorrilla": "Gemelos", "gemelo": "Gemelos", "calf": "Gemelos", "elevación de talones": "Gemelos",
  // ─ Core
  "abdominales": "Core", "plancha": "Core", "plank": "Core", "crunch": "Core",
  "rueda": "Core", "core": "Core", "oblicuos": "Core",
...
function SubtabMusculo({ athleteId, range }: { athleteId: string; range: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/athletes/${athleteId}/training-stats?range=${range}`)
      .then((r) => r.ok ? r.json() : null)
      .then((d: { sessions: TrainingSession[] } | null) => { setData(d?.sessions ?? []); setLoading(false); })
      .catch(() => { setData([]); setLoading(false); });
  }, [athleteId, range]);

  if (loading) return <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-20 rounded-2xl" />)}</div>;
  ...
  const muscleOrder = ["Pectoral", "Espalda", "Trapecio", "Deltoides", "Bíceps", "Tríceps", "Cuádriceps", "Isquiotibiales", "Glúteos", "Gemelos", "Core", "Otros"];
  ...
  const allWeeks = [...new Set(data.flatMap((s) => {
    const d = new Date(s.date);
    const jan4 = new Date(d.getFullYear(), 0, 4);
    const wk = Math.ceil(((d.getTime() - jan4.getTime()) / 86_400_000 + jan4.getDay() + 1) / 7);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatBig label="Series totales" value={String(Object.values(vol).reduce((a, v) => a + v.sets, 0))} />
        <StatBig label="Tonelaje total" value={`${Math.round(Object.values(vol).reduce((a, v) => a + v.tonnage, 0) / 1000).toLocaleString("es-ES")} t`} />
      ...
      <ChartCard title="Series por grupo muscular" footnote={`${data.length} sesiones analizadas`}>
        <ResponsiveContainer width="100%" height={Math.max(200, sortedMuscles.length * 36)}>
          <BarChart data={seriesByMuscle} layout="vertical" margin={{ top: 4, right: 40, left: 10, bottom: 0 }}>
            <CartesianGrid {...CHART_GRID} horizontal={false} />
            <XAxis type="number" tick={AXIS_STYLE} axisLine={false} tickLine={false} />
            <YAxis type="category" dataKey="name" tick={{ fontSize: 12, fill: "rgba(255,255,255,0.7)" }} axisLine={false} tickLine={false} width={70} />
            <Tooltip {...TOOLTIP_STYLE} formatter={(v) => [`${v} series`, "Volumen"]} />
            <Bar dataKey="series" name="Series" radius={[0, 6, 6, 0]}>
              {seriesByMuscle.map((entry) => (
                <Cell key={entry.name} fill={MUSCLE_STROKE[entry.name] ?? "#94a3b8"} fillOpacity={0.8} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      ...
      <ChartCard title="Tonelaje por grupo muscular (kg)" footnote="kg × repeticiones totales">
        <ResponsiveContainer width="100%" height={Math.max(200, sortedMuscles.length * 36)}>
          <BarChart data={seriesByMuscle} layout="vertical" margin={{ top: 4, right: 40, left: 10, bottom: 0 }}>
            ...
            <YAxis type="category" dataKey="name" tick={{ fontSize: 12, fill: "rgba(255,255,255,0.7)" }} axisLine={false} tickLine={false} width={70} />
            <Tooltip {...TOOLTIP_STYLE} formatter={(v) => [`${Number(v).toLocaleString("es-ES")} kg`, "Tonelaje"]} />
            <Bar dataKey="tonelaje" name="Tonelaje" fill="#f59e0b" radius={[0, 6, 6, 0]} opacity={0.75} />
          </BarChart>
        </ResponsiveContainer>
      </ChartCard>
        ...
        <ChartCard title="Series semanales por músculo">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr>
                  <th className="py-1.5 pr-4 text-left font-medium text-foreground/40">Músculo</th>
                  {allWeeks.map((w) => (
                    <th key={w} className="px-2 py-1.5 text-center font-medium text-foreground/40">{w.split("-")[1]}</th>
                  ))}
                  <th className="pl-3 py-1.5 text-right font-medium text-foreground/40">Total</th>
                  <th className="pl-3 py-1.5 text-center font-medium text-foreground/40">MEV</th>
                  <th className="pl-3 py-1.5 text-center font-medium text-foreground/40">MAV</th>
                  <th className="pl-3 py-1.5 text-center font-medium text-foreground/40">MRV</th>
                </tr>
              </thead>
              <tbody>
                {sortedMuscles.map((muscle) => {
                  const refs = VOLUME_REFS[muscle];
                  const weeklyAvg = allWeeks.length > 0
                    ? Math.round(allWeeks.reduce((s, w) => s + (weeklyByMuscle[muscle]?.[w] ?? 0), 0) / allWeeks.length)
                    : 0;
                  const zone = refs
                    ? weeklyAvg < refs.mev ? "danger" : weeklyAvg < refs.mav ? "warning" : weeklyAvg < refs.mrv ? "success" : "warn-over"
                    : "none";
                  return (
                  <tr key={muscle} className="border-t border-line/20">
                    <td className="py-2 pr-4">
                      <span className="flex items-center gap-2">
                        <span className="inline-block h-2.5 w-2.5 rounded-full" style={{ background: MUSCLE_STROKE[muscle] ?? "#94a3b8" }} />
                        {muscle}
                      </span>
                    </td>
                    {allWeeks.map((w) => {
                        ...
                        <td key={w} className="px-2 py-2 text-center">
                          {v > 0 ? (
                            <span className={`inline-block rounded px-1.5 py-0.5 font-semibold ${
                              v >= 15 ? "bg-success/20 text-success" : v >= 10 ? "bg-warning/20 text-warning" : "bg-surface-strong"
                            }`}>{v}</span>
                          ) : (
                    ...
                    <td className="pl-3 py-2 text-right font-semibold">{vol[muscle]?.sets ?? 0}</td>
                    <td className={`pl-3 py-2 text-center font-semibold ${zone === "danger" ? "text-danger" : zone === "warning" ? "text-warning" : "text-foreground/50"}`}>{refs?.mev ?? "—"}</td>
                    <td className={`pl-3 py-2 text-center font-semibold ${zone === "success" ? "text-success" : "text-foreground/50"}`}>{refs?.mav ?? "—"}</td>
                    <td className={`pl-3 py-2 text-center font-semibold ${zone === "warn-over" ? "text-danger" : "text-foreground/40"}`}>{refs?.mrv ?? "—"}</td>
                  </tr>
                  );
                })}
              </tbody>
          ...
          <div className="mt-3 flex flex-wrap gap-3 text-xs text-foreground/40">
            <span><span className="font-medium text-success">Verde</span> ≥15 series (≈MAV)</span>
            <span><span className="font-medium text-warning">Naranja</span> ≥10 (≈MEV)</span>
            <span className="ml-2"><strong>MEV</strong> mín efectivo · <strong>MAV</strong> máx adaptativo · <strong>MRV</strong> máx recuperable (Israetel)</span>
          </div>
        </ChartCard>
      )}
    </div>
    ...
    <div className="rounded-2xl border border-line bg-surface p-5 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          {plan.isActive && (
            <span className="inline-block rounded-full bg-success/15 px-2 py-0.5 text-xs font-medium text-success">Activo</span>
          )}
        </div>
        {plan.kcalTarget ? (
          <span className="text-2xl font-bold">{plan.kcalTarget.toLocaleString("es-ES")} <span className="text-sm font-normal text-foreground/50">kcal</span></span>
        ) : null}
      </div>
          ...
          <div className="shrink-0">
            <ResponsiveContainer width={160} height={160}>
              <PieChart>
                ...
                <Tooltip
                  contentStyle={{ background: "var(--color-surface)", border: "1px solid var(--color-line)", borderRadius: 10, fontSize: 12 }}
                  formatter={(v, name) => [`${String(v)}g (${totalG > 0 ? Math.round((Number(v) / totalG) * 100) : 0}%)`, String(name)]}
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="flex-1 space-y-2">
          {[
            ...
            <div key={m.label} className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <div className="h-2 w-24 rounded-full bg-line/20">
                  <div className="h-2 rounded-full" style={{ background: m.color, width: totalG > 0 ? `${Math.round((m.value / totalG) * 100)}%` : "0%" }} />
                </div>
          ...
          {plan.kcalTarget && totalG > 0 && (
            <p className="mt-1 text-xs text-foreground/40">
              {Math.round(totalG * 4)} kcal desde macros · P:{Math.round((p / totalG) * 100)}% C:{Math.round((c / totalG) * 100)}% G:{Math.round((f / totalG) * 100)}%
            </p>
          )}
        </div>
      </div>
      ...
      {plan.meals.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-widest text-foreground/35">{plan.meals.length} comidas</p>
          {[...plan.meals].sort((a, b) => a.order - b.order).map((meal) => {
            ...
            return (
              <div key={meal.id} className="rounded-xl border border-line bg-surface-strong overflow-hidden">
                <button
                  type="button"
                  onClick={() => setExpanded(isExp ? null : meal.id)}
                  className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm hover:bg-line/10 transition"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="font-medium truncate">{meal.name}</span>
                    {meal.time && <span className="shrink-0 text-xs text-foreground/40">{meal.time}</span>}
                  </div>
                  <div className="shrink-0 flex items-center gap-3 text-xs text-foreground/50">
                ...
                {isExp && meal.foods.length > 0 && (
                  <div className="border-t border-line/30 divide-y divide-line/20">
                    {meal.foods.map((fd) => (
                      <div key={fd.id} className="flex items-center justify-between px-4 py-2 text-xs">
                        <span className="text-foreground/80">{fd.food} <span className="text-foreground/40">{fd.quantity}{fd.unit}</span></span>
                        <div className="flex gap-3 text-foreground/50">
                          {fd.kcal != null && <span>{fd.kcal} kcal</span>}
                          {fd.proteinG != null && <span className="text-success">{fd.proteinG}P</span>}
                          {fd.carbsG != null && <span className="text-sky-400">{fd.carbsG}C</span>}
...
function aggregateByDay(logs: NutritionLogEntry[]): DailyNutrition[] {
  ...
  for (const log of logs) {
    ...
    const existing = map.get(day) ?? {
      ...
      kcal: 0, proteinG: 0, carbsG: 0, fatG: 0, entries: 0,
    };
...
function NutritionLogCharts({ logs, activePlan }: { logs: NutritionLogEntry[]; activePlan: OverviewNutritionPlan | null }) {
  const daily = aggregateByDay(logs);

  if (daily.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-line bg-surface py-10 text-center text-sm text-foreground/40">
  ...
  const compliance = kcalTarget && kcalTarget > 0
    ? Math.round((daily.filter((d) => d.kcal >= kcalTarget * 0.85 && d.kcal <= kcalTarget * 1.15).length / daily.length) * 100)
    : null;
  ...
  return (
    <div className="space-y-4">
      {/* KPIs */}
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatBig label="Kcal media/día" value={`${avgKcal.toLocaleString("es-ES")}`} sub={kcalTarget ? `Obj: ${kcalTarget}` : undefined} />
        <StatBig label="Proteína media" value={`${avgProtein}g`} sub={proteinTarget ? `Obj: ${proteinTarget}g` : undefined} />
        <StatBig label="Días registrados" value={String(daily.length)} />
        {compliance !== null && <StatBig label="Cumplimiento kcal" value={`${compliance}%`} sub="±15% del objetivo" />}
      </div>

      {/* Kcal por día */}
      <ChartCard title="Calorías por día" footnote={kcalTarget ? `Línea punteada = objetivo ${kcalTarget} kcal` : undefined}>
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={daily} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
            ...
            <CartesianGrid {...CHART_GRID} />
            <XAxis dataKey="label" tick={{ fontSize: 10, fill: "rgba(255,255,255,0.4)" }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
            <YAxis tick={AXIS_STYLE} axisLine={false} tickLine={false} unit=" kcal" width={60} />
            <Tooltip {...TOOLTIP_STYLE} formatter={(v) => [`${Number(v).toLocaleString("es-ES")} kcal`, "Calorías"]} />
            {kcalTarget && (
              <ReferenceLine y={kcalTarget} stroke="#f59e0b" strokeDasharray="5 5" strokeWidth={1.5}
                label={{ value: `Obj ${kcalTarget}`, position: "insideTopRight", fill: "#f59e0b", fontSize: 10 }} />
            )}
            <Area type="monotone" dataKey="kcal" name="Kcal" stroke="#f59e0b" strokeWidth={2} fill="url(#gradKcal)" dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      </ChartCard>
      ...
      <ChartCard title="Macros por día (g)" footnote="Proteína · Carbohidratos · Grasa">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={daily} margin={{ top: 4, right: 8, left: -20, bottom: 0 }} barSize={daily.length > 30 ? 6 : 14}>
            <CartesianGrid {...CHART_GRID} />
            <XAxis dataKey="label" tick={{ fontSize: 10, fill: "rgba(255,255,255,0.4)" }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
            ...
            <Tooltip
              {...TOOLTIP_STYLE}
              formatter={(v, name) => {
                const labels: Record<string, string> = { proteinG: "Proteína", carbsG: "Carbohidratos", fatG: "Grasa" };
                return [`${Number(v).toFixed(1)}g`, labels[String(name)] ?? String(name)];
              }}
            />
            <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}
              formatter={(v: string) => ({ proteinG: "Proteína", carbsG: "Carbohidratos", fatG: "Grasa" } as Record<string, string>)[v] ?? v} />
            <Bar dataKey="proteinG"  name="proteinG"  stackId="a" fill="#22c55e" radius={[0,0,0,0]} />
      ...
      <ChartCard title="Proteína y grasa diaria (g)">
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={daily} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="label" tick={{ fontSize: 10, fill: "rgba(255,255,255,0.4)" }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
            <YAxis tick={AXIS_STYLE} axisLine={false} tickLine={false} unit="g" />
            <Tooltip {...TOOLTIP_STYLE} formatter={(v, name) => [`${Number(v).toFixed(1)}g`, String(name)]} />
            <Legend wrapperStyle={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }} />
            {proteinTarget && (
              <ReferenceLine y={proteinTarget} stroke="#22c55e" strokeDasharray="4 4" strokeWidth={1}
                label={{ value: `Obj ${proteinTarget}g`, position: "insideTopRight", fill: "#22c55e", fontSize: 10 }} />
            )}
            <Line type="monotone" dataKey="proteinG" name="Proteína" stroke="#22c55e" strokeWidth={2} dot={false} />
            <Line type="monotone" dataKey="fatG"     name="Grasa"    stroke="#f59e0b" strokeWidth={1.5} strokeDasharray="4 4" dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </ChartCard>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              <div className="shrink-0">
                <ResponsiveContainer width={150} height={150}>
                  <PieChart>
                    <Pie data={slices} dataKey="value" innerRadius={44} outerRadius={66} paddingAngle={3} startAngle={90} endAngle={-270}>
                      {slices.map((s, i) => <Cell key={s.name} fill={MACRO_COLORS[i]} />)}
                    </Pie>
                    <Tooltip {...TOOLTIP_STYLE}
                      formatter={(v, name) => [`${Number(v).toFixed(1)}g (${totalG > 0 ? Math.round((Number(v) / totalG) * 100) : 0}%)`, String(name)]} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
              <div className="flex-1 space-y-2">
                {slices.map((s) => (
                  <div key={s.name} className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="h-2 w-24 rounded-full bg-line/20">
                        <div className="h-2 rounded-full" style={{ background: s.color, width: totalG > 0 ? `${Math.round((s.value / totalG) * 100)}%` : "0%" }} />
                      </div>
                      <span className="w-16 text-right text-sm font-semibold">{s.value}g · {totalG > 0 ? Math.round((s.value / totalG) * 100) : 0}%</span>
                    </div>
                  </div>
                ))}
                <p className="mt-1 text-xs text-foreground/40">Media de {daily.length} días · {avgKcal.toLocaleString("es-ES")} kcal/día</p>
              </div>
            </div>
          </ChartCard>
        );
...
function SubtabNutricion({ overview }: { overview: Overview }) {
  ...
  useEffect(() => {
    fetch(`/api/nutrition-logs?athleteId=${overview.id}&take=90`)
      .then((r) => r.ok ? r.json() : null)
      .then((d: { items: NutritionLogEntry[] } | null) => { setLogs(d?.items ?? []); setLoadingLogs(false); })
      ...
      {activePlan ? (
        <>
          <div className="mt-2 border-t border-line/30 pt-4">
            <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-foreground/35">Plan nutricional asignado</p>
          </div>
          <NutritionPlanSummary plan={activePlan} />
          {others.map((p) => <NutritionPlanSummary key={p.id} plan={p} />)}
        </>
      ) : (
        <div className="rounded-2xl border border-dashed border-line bg-surface py-10 text-center text-sm text-foreground/40">
...
function EstadisticasTab({ overview }: { overview: Overview }) {
  const [subtab, setSubtab] = useState<"composicion" | "adherencia" | "entrenamiento" | "musculo" | "nutricion">("composicion");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex gap-1 rounded-xl bg-surface-strong p-1">
          {subtabs.map((st) => (
            <button
              key={st.key}
              type="button"
        ...
        <select
          ...
          className="rounded-xl border border-line bg-surface px-3 py-1.5 text-xs text-foreground focus:outline-none focus:border-accent/50"
...
function ResumenTab({ overview }: { overview: Overview }) {
  ...
  return (
    <div className="space-y-6">
      {daysNoCI != null && daysNoCI > 7 && (
        <div className="flex items-center gap-3 rounded-2xl border border-warning/30 bg-warning/8 px-4 py-3 text-sm text-warning">
          <span>⚠️</span>
          <span>Sin check-in hace <strong>{daysNoCI} días</strong>. Puede necesitar seguimiento.</span>
        </div>
      )}
        ...
        {lastCI ? (
          <div className="rounded-2xl border border-line bg-surface p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="text-xs text-foreground/40">{lastCI.weekLabel ?? lastCI.date}</span>
            </div>
            <AdherenceBar pct={lastCI.adherencePct} />
            ...
            {lastCI.sensations && (
              <p className="mt-3 rounded-xl border border-line bg-surface-strong px-3 py-2 text-xs italic text-foreground/55">
        ...
        {activePlan ? (
          <div className="rounded-2xl border border-line bg-surface p-5">
            <div className="mb-1 flex items-center justify-between">
              ...
              <Link href={`/coach/plans/${activePlan.id}/print`}
                className="text-xs text-accent hover:underline">Imprimir</Link>
            </div>
            <p className="font-medium">{activePlan.title}</p>
            {activePlan.weekLabel && <p className="text-xs text-foreground/50">{activePlan.weekLabel}</p>}
            <p className="mt-2 text-xs text-foreground/50">{activePlan.sessionsCount} sesiones</p>
          </div>
        ) : (
...
function CheckInsTab({
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-foreground/70">Historial de check-ins</h3>
        <span className="text-xs text-foreground/40">{checkIns.length} registros</span>
      </div>
        ...
        <div className="max-h-[680px] space-y-3 overflow-y-auto pr-1">
          {checkIns.map((ci) => {
            const adhClass = ci.adherencePct >= 80
              ? "text-success"
              : ci.adherencePct >= 60 ? "text-warning" : "text-danger";
            const isEditing = editingId === ci.id;
            const savedNote = notes[ci.id] ?? "";
            return (
              <div key={ci.id} className="rounded-2xl border border-line bg-surface p-4">
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <p className="font-semibold">{ci.weekLabel ?? ci.date}</p>
                    <p className="text-xs text-foreground/40">{ci.date}</p>
                  </div>
                ...
                {ci.sensations && (
                  <p className="mt-2 rounded-xl border border-line bg-surface-strong px-3 py-2 text-xs italic text-foreground/50">
                ...
                {isEditing ? (
                  <div className="mt-3 space-y-2">
                    <textarea
                      ...
                      className="w-full resize-none rounded-xl border border-line bg-background px-3 py-2 text-xs outline-none focus:border-accent"
                      rows={2}
                      placeholder="Nota del coach..."
                    />
                    <div className="flex gap-2">
                      <button onClick={() => void saveNote(ci.id)} disabled={saving}
                        className="rounded-full bg-accent px-3 py-1 text-xs font-semibold text-white disabled:opacity-50">
                        {saving ? "..." : "Guardar"}
                      </button>
                      <button onClick={() => setEditingId(null)}
                        className="rounded-full border border-line px-3 py-1 text-xs text-foreground/50 hover:text-foreground">
                        Cancelar
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="mt-2 space-y-1">
                    {savedNote && (
                      <div className="rounded-xl border border-accent/20 bg-accent/5 px-3 py-2">
                        <p className="text-xs font-semibold text-accent/70">Nota coach</p>
                        <p className="text-xs text-foreground/65">{savedNote}</p>
                      </div>
                    )}
                    <button
                      onClick={() => { setEditingId(ci.id); setNoteText(savedNote); }}
                      className="text-xs text-accent/60 transition hover:text-accent">
...
function TrainingTab({ overview }: { overview: Overview }) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-semibold text-foreground/70">Planes de entrenamiento</h3>
      {plans.length === 0 ? (
        <EmptyCard text="Sin planes asignados" tall />
      ) : (
        <div className="space-y-3">
          {plans.map((plan) => (
            <div key={plan.id} className="overflow-hidden rounded-2xl border border-line bg-surface">
              <button
                onClick={() => setExpanded(expanded === plan.id ? null : plan.id)}
                className="flex w-full items-center justify-between gap-3 px-5 py-4 text-left transition hover:bg-surface-strong"
              >
                <div>
                  <p className="font-semibold">{plan.title}</p>
                  <p className="text-xs text-foreground/40">
                    {plan.weekLabel ?? plan.createdAt} · {plan.sessionsCount} sesiones
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-3">
                  <Link href={`/coach/plans/${plan.id}/print`}
                    onClick={(e) => e.stopPropagation()}
                    className="rounded-full border border-line px-3 py-1 text-xs transition hover:border-accent/40 hover:text-accent">
                    Imprimir
                  </Link>
              ...
              {expanded === plan.id && (
                <div className="border-t border-line px-5 py-4 text-sm text-foreground/60">
          ...
          {connections.map((c) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold">{PROVIDER_LABELS[c.provider] ?? c.provider}</p>
                  <p className="text-xs text-foreground/50">
                    {c.isActive ? "Activa" : "Inactiva"}
                    {c.lastSyncAt ? ` · Sync ${new Date(c.lastSyncAt).toLocaleDateString("es-ES")}` : ""}
                  </p>
                </div>
              </div>
            </div>
...
function ConsentTab({ athleteId }: { athleteId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {consent == null ? (
        <div className="rounded-2xl border border-warning/40 bg-warning/8 p-5 text-sm">
          <p className="font-semibold text-warning">⚠️ Contrato no aceptado</p>
          <p className="mt-1 text-foreground/60">
            El atleta aún no ha firmado el contrato de coaching. Se solicitará durante el onboarding.
          </p>
        </div>
      ) : (
        <div className="rounded-2xl border border-success/40 bg-success/5 p-5 space-y-3">
          ...
          {consent.signatureRef && (
            <div className="rounded-xl border border-line bg-surface-strong px-4 py-2 text-sm">
...
function AthleteDetailContent() {
  ...
  const tabsWithBadges = TABS.map((t) => ({
    ...
    label: t.label,
    badge:
      t.key === "checkins" ? overview.stats.totalCheckIns :
      t.key === "documentos" ? overview.documents.length :
      undefined,
  }));
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <Link href="/coach/athletes"
          className="flex items-center gap-1.5 text-xs text-foreground/40 transition hover:text-foreground">
          ← Todos los atletas
        </Link>
        <button
          onClick={() => setAsideOpen(false)}
          className="rounded-lg p-1 text-foreground/30 transition hover:bg-surface-strong hover:text-foreground"
  ...
  const main = (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4">
        ...
        {!asideOpen && (
          <button
            ...
            className="hidden shrink-0 rounded-lg border border-line p-1.5 text-foreground/40 transition hover:bg-surface-strong hover:text-foreground xl:flex"
      ...
      <TabsBar
        ...
        active={activeTab}
        onChange={setTab}
        className="sticky top-16 z-10 -mx-1 bg-background/95 px-1 py-2 backdrop-blur-sm"
      />
