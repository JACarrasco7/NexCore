  ...
  return (
    <main className="mx-auto flex w-full max-w-[1480px] flex-1 flex-col gap-8 px-6 py-8 md:px-10 lg:px-12">
      <SectionIntro
        ...
        aside={result ? `${result.detectedDelimiter} · ${result.rows.length} ejercicios` : undefined}
      />
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm">
        ...
        <Link href="/coach/import-lab/nutrition" className="rounded-full border border-line px-4 py-1.5 text-foreground/70 hover:border-accent/30 hover:text-accent">
          Import nutrición
        </Link>
      </div>

      {/* Column reference */}
      <details className="rounded-3xl border border-line bg-surface-strong">
        <summary className="cursor-pointer select-none px-5 py-3 text-sm font-medium text-foreground/70 hover:text-foreground">
        ...
        <div className="px-5 pb-5">
          <div className="mt-3 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              { col: "tecnica", desc: "drop_set, rest_pause, myo_reps, super_set, giant_set, cluster, tempo, pre_fatiga, eccentrico, isometrico, amrap" },
              ...
              <div key={col} className="rounded-xl border border-line bg-surface px-3 py-2">
                <code className="font-semibold text-accent">{col}</code>
                <p className="mt-0.5 leading-tight text-foreground/50">{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        ...
        <article className="flex flex-col gap-4 rounded-4xl border border-line bg-surface p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-foreground/60">Etiqueta de semana</label>
              <input
                type="text"
                value={weekLabel}
          ...
          <div className="flex items-center gap-4">
            ...
            {!selectedAthleteId && result && (
              <span className="text-xs text-foreground/40">Selecciona un atleta para guardar</span>
            )}
          </div>
        </article>
        ...
        <article className="max-h-[80vh] overflow-auto rounded-4xl border border-slate-700/50 bg-slate-950 p-6 text-slate-100">
          ...
          {!result ? (
            <p className="text-sm text-slate-400">Pega contenido en el panel izquierdo para ver el resultado.</p>
          ) : (
            <div className="space-y-6">
              {result.plan.sessions.map((session) => (
                <div key={session.id} className="rounded-3xl border border-slate-700/50 bg-slate-900/60 p-5">
                  <div className="mb-4 flex items-center justify-between gap-4">
                    <h3 className="text-base font-bold">{session.name}</h3>
                    <span className="shrink-0 rounded-full border border-slate-600/50 px-3 py-1 text-xs uppercase tracking-widest text-slate-400">
                  ...
                  <div className="space-y-3">
                    {session.exercises.map((ex, ei) => {
                      ...
                      const techColor = TECHNIQUE_COLORS[techKey] ?? "bg-slate-700/30 text-slate-300 border-slate-600/40";
                      ...
                      return (
                        <div key={ei} className="space-y-2 rounded-2xl border border-slate-700/40 bg-slate-900/50 p-4">
                          ...
                          {ex.coachCue && (
                            <div className="rounded-xl border border-slate-600/30 bg-slate-800/50 px-3 py-2 text-xs italic text-slate-400">
                          ...
                          {ex.notes && (
                            <p className="text-xs text-slate-400/70">{ex.notes}</p>
                          )}
                        </div>
                      );
                    })}
