...
export default function NotificationDetail({ notificationId }: { notificationId: string }) {
  ...
  return (
    <div className="p-4">
      ...
      <p className="text-foreground/50 text-xs">Título: {data.title}</p>
      <div className="mt-3 space-y-2">
        {data.deliveries && data.deliveries.length > 0 ? (
          data.deliveries.map((d: Delivery) => (
            <div key={d.id} className="border-line rounded-lg border p-3">
              ...
              {d.errorMessage && (
                <div className="text-danger mt-2 text-xs">Error: {d.errorMessage}</div>
              )}
              {d.externalId && (
                <div className="text-foreground/45 mt-1 text-[11px]">
                  ID externo: {d.externalId}
                </div>
              )}
            </div>
          ))
        ) : (
          <div className="text-foreground/50 text-sm">Sin entregas registradas.</div>
        )}
      </div>
    </div>
  )
