# Tech Debt & Mejoras — Apex Coach OS
*Fecha de revisión: 7 mayo 2026 · Revisión: auditoría post-Fase 8 (TOTP)*
...
Con helper:
```ts
function encodeBackupCodes(hashed: string[]) { return JSON.stringify(hashed); }
function decodeBackupCodes(raw: string | null): string[] { return raw ? JSON.parse(raw) : []; }
```
...
*Generado automáticamente por auditoría técnica · 07/05/2026*
