...
const authenticator =
  otplib.authenticator || (otplib.default && otplib.default.authenticator) || otplib
...
async function main() {
  ...
  try {
    ...
    const secret = authenticator.generateSecret
      ...
      : require('crypto').randomBytes(10).toString('hex').toUpperCase()
    let otpauth = ''
    if (typeof authenticator.keyuri === 'function') {
      otpauth = authenticator.keyuri(email, 'NEXUM', secret)
    } else {
      otpauth = `otpauth://totp/${encodeURIComponent(email)}?secret=${encodeURIComponent(secret)}&issuer=${encodeURIComponent('NEXUM')}`
    }
    ...
    try {
      ...
      console.log(
        '[debug] authenticator keys:',
        authenticator ? Object.keys(authenticator) : 'no-authenticator'
      )
    } catch (e) {
